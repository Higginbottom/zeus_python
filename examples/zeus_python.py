...
python_ver="/home/nsh2m14/python/bin/py82"
...
for i in range(istart,10000):  #We will permit up to 500 calls to python (this is a lot)
	...
	if py_cycles==3: #This is the first time thruogh - so no restart""
		cmdline="mpirun -n 512 "+python_ver+" -z  input.pf > "+hdf_file[2:]+".out"  #We now run python
	else:
		cmdline="mpirun -n 512 "+python_ver+" -z -r  input.pf > "+hdf_file[2:]+".out"  #We now run python
